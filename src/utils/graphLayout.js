...
export function computeLayout(commits, branches, HEAD) {
  ...
  // Each group becomes a single bubble above the commit node.
  const labelsByCommit = {}

  for (const [name, sha] of Object.entries(branches)) {
    if (!labelsByCommit[sha]) labelsByCommit[sha] = []
    labelsByCommit[sha].push(name)
  }

  const branchLabelGroups = []
  for (const [sha, names] of Object.entries(labelsByCommit)) {
